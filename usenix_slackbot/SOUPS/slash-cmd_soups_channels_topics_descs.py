...
slack_event_adapter = SlackEventAdapter(
    os.environ['SOUPS_SIGNING_SECRET'],'/slack/events',app)

client = slack.WebClient(token=os.environ['SOUPS_SLACK_TOKEN'])
BOT_ID = client.api_call("auth.test")['user_id']
user = slack.WebClient(token=os.environ['SOUPS_USERSLACK_TOKEN'])
...
def create_channels():
    ...
    list_of_channels = list(f.slack_channels_soups(weblink))
...
def set_topic_desc():
    ...
    new_channels = list(f.slack_channels_soups(weblink))
    existing_channels = client.conversations_list(limit=999)
    counter = 0
    channel_ids = []
    channel_names = []
    ...
    topic_strings = list(f.channel_topic_soups(weblink))
    desc_strings = list(f.channel_desc_soups(weblink))
