...
def slack_channels_atc(body):
  ...
  if len_days_tracks < len_days_sessions:
    formatted_tracks_sessions = formatted_list + formatted_days_sessions_in_order[len_days_tracks:]
  formatted_with_sessions = []
  
  #Formats tracks.
  for x in formatted_tracks_sessions:
    if 'day' in x:
        counter = 0
        secondcounter=1
    else:
